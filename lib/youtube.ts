...
import { parseSongTitle } from "./parse-title";
...
async function fetchViaDataApi(
  ...
  let playlistTitle = "Shyama Sangeet Collection";
    ...
    for (const item of data.items ?? []) {
      ...
      tracks.push(
        toTrack(
          videoId,
          title,
          snippet?.channelTitle || "Shyama Sangeet",
...
async function fetchViaRss(id: string): Promise<PlaylistData> {
  ...
  for (const match of entries) {
    ...
    tracks.push(toTrack(videoId, title, author || "Shyama Sangeet", tracks.length, thumb));
  }
...
export async function getPlaylist(): Promise<PlaylistData> {
  ...
  try {
    ...
    return {
      playlistId: id,
      title: "Shyama Sangeet Collection",
      tracks: [],
      source: "rss",
    };
  }
