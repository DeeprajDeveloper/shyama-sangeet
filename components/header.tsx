"use client";

import { useEffect, useState } from "react";
import { SITE_NAME, SITE_TAGLINE_BN, KALI_PUJA_DATES, type KaliPujaDate } from "@/lib/constants";

function YtIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M23.5 6.2a3 3 0 0 0-2.1-2.1C19.5 3.6 12 3.6 12 3.6s-7.5 0-9.4.5A3 3 0 0 0 .5 6.2C0 8.1 0 12 0 12s0 3.9.5 5.8a3 3 0 0 0 2.1 2.1c1.9.5 9.4.5 9.4.5s7.5 0 9.4-.5a3 3 0 0 0 2.1-2.1C24 15.9 24 12 24 12s0-3.9-.5-5.8ZM9.6 15.6V8.4L15.8 12l-6.2 3.6Z" />
    </svg>
  );
}

type ClockParts = { hh: string; mm: string; ampm: string } | null;

function useClock(): ClockParts {
  const [parts, setParts] = useState<ClockParts>(null);

  useEffect(() => {
    function tick() {
      const now = new Date();
      let hours = now.getHours();
      const minutes = now.getMinutes();
      const ampm = hours >= 12 ? "PM" : "AM";
      hours = hours % 12 || 12;
      setParts({
        hh: String(hours).padStart(2, "0"),
        mm: String(minutes).padStart(2, "0"),
        ampm,
      });
    }
    tick();
    const id = setInterval(tick, 1_000);
    return () => clearInterval(id);
  }, []);

  return parts;
}

function useDateRotator(dates: KaliPujaDate[]): KaliPujaDate | null {
  const [idx, setIdx] = useState(0);

  useEffect(() => {
    if (dates.length <= 1) return;
    const id = setInterval(() => setIdx((i) => (i + 1) % dates.length), 15_000);
    return () => clearInterval(id);
  }, [dates.length]);

  return dates[idx] ?? null;
}

type CountdownTarget = {
  label: string;
  days: number;
  hours: number;
  minutes: number;
} | null;

/** Always counts down to the nearest upcoming date in the array. */
function useNextCountdown(dates: KaliPujaDate[]): CountdownTarget {
  const [target, setTarget] = useState<CountdownTarget>(null);

  useEffect(() => {
    function calc() {
      const now = Date.now();
      const next = dates
        .map((d) => ({ label: d.en, ms: new Date(d.en).getTime() }))
        .filter((d) => d.ms > now)
        .sort((a, b) => a.ms - b.ms)[0];

      if (!next) { setTarget(null); return; }

      const diff = next.ms - now;
      setTarget({
        label:   next.label,
        days:    Math.floor(diff / 86_400_000),
        hours:   Math.floor((diff % 86_400_000) / 3_600_000),
        minutes: Math.floor((diff % 3_600_000)  / 60_000),
      });
    }
    calc();
    const id = setInterval(calc, 60_000);
    return () => clearInterval(id);
  }, [dates]);

  return target;
}

export function Header({ playlistId }: { playlistId?: string }) {
  const clock       = useClock();
  const pujaDate    = useDateRotator(KALI_PUJA_DATES);
  const countdown   = useNextCountdown(KALI_PUJA_DATES);
  const ytUrl = playlistId
    ? `https://www.youtube.com/playlist?list=${playlistId}`
    : "https://www.youtube.com";

  return (
    <header className="site-header">
      <div className="site-header__left">
        {clock && (
          <time className="site-header__clock">
            {clock.hh}<span className="clock-colon">:</span>{clock.mm} {clock.ampm}
          </time>
        )}

        <div className="site-header__puja-stack">
          {pujaDate && (
            <div className="site-header__puja-date" key={pujaDate.en}>
              <span className="site-header__puja-date-label">Kali Puja</span>
              <span className="site-header__puja-date-en">{pujaDate.en}</span>
              {pujaDate.bn && (
                <span className="site-header__puja-date-bn">{pujaDate.bn}</span>
              )}
            </div>
          )}

          {countdown && (
            <div className="site-header__countdown">
              <span className="site-header__countdown-label">Next Puja</span>
              <span className="site-header__countdown-units">
                <span className="site-header__countdown-unit">
                  <strong>{countdown.days}</strong>
                  <span>d</span>
                </span>
                <span className="site-header__countdown-sep">:</span>
                <span className="site-header__countdown-unit">
                  <strong>{countdown.hours}</strong>
                  <span>h</span>
                </span>
                <span className="site-header__countdown-sep">:</span>
                <span className="site-header__countdown-unit">
                  <strong>{countdown.minutes}</strong>
                  <span>m</span>
                </span>
              </span>
            </div>
          )}
        </div>
      </div>

      <div className="site-header__center">
        <p className="site-header__tagline">{SITE_TAGLINE_BN}</p>
      </div>

      <div className="site-header__right">
        <a
          href={ytUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="site-header__yt-link"
          aria-label="YouTube Playlist"
          title={`Open ${SITE_NAME} YouTube Playlist`}
        >
          <span className="site-header__yt-link-text">YouTube Playlist</span>
          <YtIcon />
        </a>
      </div>
    </header>
  );
}
