...
function placeholderTrack(videoId: string, position: number): Track {
  return {
    videoId,
    title: `গান ${position + 1}`,
    displayTitle: `গান ${position + 1}`,
    thumbnail: thumbnailUrl(videoId),
    channelTitle: "Shyama Sangeet",
    position,
  };
}

function isPlaceholder(track: Track) {
  return /^গান \d+$/.test(track.displayTitle);
}
      ...
      setTracks((prev) =>
        prev.map((track) => {
          ...
          return {
            ...track,
            title: extra.title,
            displayTitle: extra.displayTitle || parseSongTitle(extra.title).displayTitle,
